...
from api.dialogflowApi import detect_intent_via_event
from api.telegramApi import send_message, send_message_with_options
...
def __display_default_response(user: User, intent_result, session_id):
    ...
    return send_message(user, intent_result, session_id, response)
...
def __display_main_greeting(user: User, intent_result, session_id):
    ...
    return send_message_with_options(user, intent_result, session_id, response, *MAIN_SUGGESTIONS, row_width=1)


# Displays response with menu items offered as a bulleted list
def __show_menu_response(user: User, intent_result, session_id):
    response = "Of course! Here's what we offer:\n"
...
def __show_menu_options(user: User, intent_result, session_id):
    ...
    if is_not_blank(response):
        return send_message_with_options(user, intent_result, session_id, response,
                                         *list(MENU_CODES_TO_OPTIONS.values()))
    else:
        return send_message(user, intent_result, session_id, DEFAULT_ERROR_MESSAGE)
...
def __show_orders(user: User, intent_result, session_id):
    orders = orderApi.list_orders(user)
...
def __update_order(user: User, intent_result, session_id):
    ...
    if len(order_items) > 0:
        ...
        return send_message_with_options(user, intent_result, session_id, response, "I'm done!")
    else:
...
def __confirm_order(user: User, intent_result, session_id):
    ...
    if len(order_items) > 0:
        ...
        detect_intent_via_event(session_id, 'NINJA_CAFE_RESET_TO_MAIN_EVENT')
...
def __submit_order(user: User, intent_result, session_id):
    ...
    orderApi.create_order(user, order_items)
    clear_from_order(user, session_id)
...
def __show_ongoing_order_suggestions(user: User, intent_result, session_id):
    ...
    return send_message_with_options(user, intent_result, session_id, response, *ONGOING_ORDER_SUGGESTIONS, row_width=1)
